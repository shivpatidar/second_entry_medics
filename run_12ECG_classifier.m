function [score, label] = run_12ECG_classifier(data,header_data,classes, model)
   ...
   [fb_v1, fb_lead1, st_lead1, avb_feat, lbbb_feat, normalstd_feat] = get_12ECG_features(data,header_data);
    [labelaf, scoreaf]=predict(model{1,1},fb_lead1);
    [labelavb, scoreavb]=predict(model{2,1},avb_feat);
    [labellbbb, scorelbbb]=predict(model{3,1},lbbb_feat);
    [labelnormal, scorenormal]=predict(model{4,1},normalstd_feat);
    [labelpac, scorepac]=predict(model{5,1},lbbb_feat);
    [labelpvc, scorepvc]=predict(model{6,1},lbbb_feat);
    [labelrbbb, scorerbbb]=predict(model{7,1},fb_v1);
    [labelstd, scorestd]=predict(model{8,1},normalstd_feat);
    [labelste, scoreste]=predict(model{9,1},st_lead1);
